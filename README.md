...
### Notas Adicionales
- Todo el código incluye comentarios detallados que explican cada paso y justifican decisiones.

---

### Conclusión

Para lograr una predicción precisa de las tarifas, se preprocesó el dataset.
...
Se generó la variable distancia usando las coordenadas de inicio y fin de cada viaje, aplicando distancia Manhattan, siendo esta la variable predictora más importante de nuestros modelos. 
Durante el proceso, eliminamos un 2.25% de los datos por tener mas de una variable nula o absurda, o estar fuera del alcance del desarrollo (Ciudad de Nueva York).
También se imputaron 3133 datos (0.09% del dataset) que presentaban valores absurdos, NaN o atípicos solo en una variable.

Se construyeron modelos de regresión lineal. Usando OLS y métodos de descenso por gradiente, en estos útlimos se ajustaron los hiperparámetros hasta alcanzar convergencia y métricas aceptables; aunque naturalmente peores (o iguales) que el método cerrado OLS.
En la etapa de regularización, el modelo Ridge mostró una leve variación en los coeficientes, pero sin cambios significativos en las métricas. Con Lasso y ElasticNet, no se obtuvieron mejoras (α=0).
